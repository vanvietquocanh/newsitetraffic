...
SortItems.prototype.createHtml = function(dataRender){
	...
	$.post('/getprofileuser', function(data, textStatus, xhr) {
		$.each(dataRender, function(index, val) {
			...
			if(sortItems.master){
					elementHtml +=      `<a class="upper-case text-block" style='background: #e67e22; color:#fff; margin-right:2px;'>${val.offeridSet}</a>`;
					elementHtml +=      `<a class="upper-case text-block" style='background: #3498db; color:#fff; margin-right:2px;'>${val.nameNetworkSet}</a>`;
			}
					elementHtml += 		`<a class="upper-case text-block">${val.countrySet}</a>
					                            </li>
					                            <li class="style-list-of-items">`;
